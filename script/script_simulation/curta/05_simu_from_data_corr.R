...
dfScenario <- readRDS("data/dfScenarioCorr.rds")
hp_row <- slar_taskid
...
res <- pbapply::pblapply(1:500,
                         FUN = function(iter){
                           res <- fct_simulation_paper(methods = c("sgbj", "gbt", "gt", "wald"),
                                                       nb_observations = vec_hp[["nb_observations"]],
                                                       ...
                                                       nperm_sGBJ = vec_hp[["nb_permutation"]]) %>%
