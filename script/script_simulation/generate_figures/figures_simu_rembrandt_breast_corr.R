...
dfres <- list.files("data/result_simu_corr/result_10187699/", recursive = T, full.names = T) %>%
  lapply(FUN = readRDS) %>%
  bind_rows() %>%
...
##### plot #####
vec_labels_type <- paste0("Type ", c("A", "B", "C", "D", "E", "F", "G", "H", "I"))
names(vec_labels_type) <- c("A", "B", "C", "D", "E", "F", "G", "H", "I")
vec_labels_case <- paste0("Case (", c("I", "II", "III", "IV", "V"), ")")
names(vec_labels_case) <- c(1:length(vec_labels_case))
