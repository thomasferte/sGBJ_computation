...
plot_breast_nb_genesets <- df_breast_cancer_pathway |> 
  ...
  ggplot(mapping = aes(x = method, fill = method, y = value, linetype = name)) +
  geom_bar(stat = "identity", position = position_dodge(), color = "black", size = .6) +
...
p_type <- survfit2(Surv(time=OVERALL_SURVIVAL_MONTHS, event=EVENT_OS) ~ DISEASE_TYPE,
  ...
  add_confidence_interval() +
  add_risktable(risktable_stats = "n.risk") +
  scale_color_manual(values = c("#8338EC", "#FFB703")) +
  scale_fill_manual(values = c("#8338EC", "#FFB703")) +
  labs(fill = "", color = "")
```
